/*
Problem - https://www.naukri.com/code360/problems/subset-sum-equal-to-k_1550954?leftPanelTab=0
...
public class Solution {
    ...
    public static boolean solve(int i, int n, int k, int[] arr, int[][] dp)
    {
        if( k == 0)
        {
            return true;
        ...
        if ( dp[i][k] != -1 )
        {
            return dp[i][k] == 0 ? true : false;
        }

        boolean pick = solve(i +1, n, k - arr[i], arr, dp);

        boolean nonPick = solve(i + 1, n, k, arr, dp);
